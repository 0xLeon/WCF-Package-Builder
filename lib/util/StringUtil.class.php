...
class StringUtil {
	...
	public static function formatDouble($double, $minDecimals = 0) {
		...
		$double = str_replace('.', PackageBuilder::getLanguage()->get('packageBuilder.global.decimalPoint'), $double);
	...
	public static function addThousandsSeparator($number) {
		if ($number >= 1000 || $number <= -1000) {
			$number = preg_replace('~(?<=\d)(?=(\d{3})+(?!\d))~', PackageBuilder::getLanguage()->get('packageBuilder.global.thousandsSeparator'), $number);
		}
